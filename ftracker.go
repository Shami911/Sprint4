...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    averageSpeedMS := meanSpeed(action, duration) * kmhInMsec
    heightInM := height / cmInM
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(averageSpeedMS, 2) / heightInM)* walkingSpeedHeightMultiplier * weight)* duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
    ...
    return (averageSpeed + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration 
}
